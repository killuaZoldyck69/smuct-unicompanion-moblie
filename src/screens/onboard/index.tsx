...
export function Onboard() {
  ...
  const handleCompleteProfile = async () => {
    ...
    try {
      ...
      await onboardStudentAPI(onboardPayload);

      // 3. Patch optional blood group
      if (bloodGroup) {
        try {
          await updateStudentProfile({ bloodGroup });
        } catch (e) {
        ...
        >
          ...
          <View style={[styles.bentoBox, { backgroundColor: "#ffdad6" }]}>
            <TouchableOpacity
              ...
              accessible={true}
              accessibilityRole="button"
              accessibilityLabel={`Blood Group: ${bloodGroup ? bloodGroups.find((b) => b.value === bloodGroup)?.label : "Not selected"}`}
            >
              <Feather
                name="droplet"
                size={20}
